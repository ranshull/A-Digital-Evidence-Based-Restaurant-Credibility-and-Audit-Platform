...
from ..views.feedback_views import RestaurantPrivateFeedbackCreateView

urlpatterns = [
    path('', RestaurantListView.as_view(), name='restaurant_list'),
    path('<int:pk>/', RestaurantDetailView.as_view(), name='restaurant_detail'),
    path('<int:pk>/feedback/', RestaurantPrivateFeedbackCreateView.as_view(), name='restaurant_private_feedback'),
